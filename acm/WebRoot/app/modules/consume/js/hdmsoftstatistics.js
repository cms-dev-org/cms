Ext.onReady(function() {
		...
		buttons		: [{
			...
			handler		: function(btn) {
				...
				addForm.getForm().submit({
					url			: '../app/hdmSoftStatistics.do?reqCode=' + method,
					method 		: 'POST',
					params		: {},
	...
	var grid = new Ext.ux.BaseGrid({
		...
		height		: 200,
		noRn		: false,
		list_url	: '../app/hdmSoftStatistics.do?reqCode=query',
		viewConfig	: {
			forceFit	: false
		},
		tbar		: [{
			...
			handler : function() {
				...
				Ext.getCmp('hdm_soft').setValue(0);
			...
			handler	: function(btn) {
				exportExcel('../app/hdmSoftStatistics.do?reqCode=exportExcel');
			}
		}, '->', {
	...
	grid.getStore().on('load', function(store, record, opt) {
		Ext.Ajax.request({
			url 	: '../app/hdmSoftStatistics.do?reqCode=countNum',
			success : function(response) {
				var data = response.responseText;
				var dataObj = eval('(' + data + ')');
				var hdmSoftTotal = dataObj.hdmSoftCount;
				Ext.getCmp('hdm_soft_total').setValue(hdmSoftTotal);
			},
	...
	function delItem() {
		...
		Ext.Msg.confirm('请确认', '确认删除选中项？', function(btn, text) {
			if (btn == 'yes') {
				//showWaitMsg();
				Ext.Ajax.request({
					url 	: '../app/hdmSoftStatistics.do?reqCode=delete',
					params 	: {ids : ids},
					success : function(response) {
	...
	function editInit() {
		...
		Ext.getCmp('hdm_soft_id').setValue(record[0].id);
