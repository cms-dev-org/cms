...
public class PropertiesFactory {
	...
	static {
		...
		try {
			InputStream is = classLoader.getResourceAsStream("tobacco.properties");
			PropertiesHelper ph = new PropertiesHelper(is);
			container.put(PropertiesFile.TOBACCO, ph);
		} catch(Exception e) {
			log.error(G4Constants.Exception_Head + "加载属性文件tobacco.properties出错！");
		}
	}
